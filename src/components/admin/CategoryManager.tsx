...
const CategoryManager = ({ tournamentId, categories, onUpdate }: CategoryManagerProps) => {
  ...
  const handleToggleVisibility = async (category: Category) => {
    try {
      const { error } = await supabase
        .from('categories')
        .update({ is_public: !category.is_public })
        .eq('id', category.id);

      if (error) throw error;

      toast.success(category.is_public ? 'Categoria ocultada do público!' : 'Categoria visível ao público!');
      onUpdate();
    } catch (error: any) {
                  ...
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    {editingId === category.id ? (
                      ...
                      <>
                        ...
                        {getStatusBadge(category.status, category.is_public ?? true)}
                      </>
                    )}
                  </div>
                  ...
                  {editingId !== category.id && (
                    <div className="flex items-center gap-2 justify-end">
                      ...
                      <Button
                        ...
                        title={category.is_public ?? true ? 'Ocultar do público' : 'Tornar pública'}
                        className="flex-shrink-0"
                      >
                        {(category.is_public ?? true) ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4 text-muted-foreground" />}
                      </Button>
                      <Button
                        variant="ghost"
                        size="icon"
